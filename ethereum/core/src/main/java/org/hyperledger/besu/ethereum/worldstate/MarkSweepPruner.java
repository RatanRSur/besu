...
public class MarkSweepPruner {
  ...
  private CompletableFuture<Void> processAccountState(
      final Bytes value, final ExecutorService executorService) {
    final StateTrieAccountValue accountValue = StateTrieAccountValue.readFrom(RLP.input(value));
    markNode(accountValue.getCodeHash());

    return createStorageTrie(accountValue.getStorageRoot())
