...
public class Pruner {
  ...
  private void handleNewBlock(final BlockAddedEvent event) {
    ...
    if (pruningPhase.compareAndSet(
      ...
      sweep().thenRun(() -> pruningPhase.compareAndSet(PruningPhase.SWEEPING, PruningPhase.IDLE));
    }
  }

  private CompletableFuture<Void> mark(final BlockHeader header) {
    final Hash stateRoot = header.getStateRoot();
    LOG.info(
        "Begin marking used nodes for pruning. Block number: {} State root: {}",
        markBlockNumber,
        stateRoot);
    return handlePruningExceptions(pruningStrategy.mark(stateRoot));
  }
  ...
  private CompletableFuture<Void> sweep() {
    ...
    return handlePruningExceptions(() -> pruningStrategy.sweepBefore(markBlockNumber));
  }

  private CompletableFuture<Void> handlePruningExceptions(final Runnable runnable) {
    return handlePruningExceptions(CompletableFuture.runAsync(runnable, executorService));
  }

  private CompletableFuture<Void> handlePruningExceptions(final CompletableFuture<Void> future) {
